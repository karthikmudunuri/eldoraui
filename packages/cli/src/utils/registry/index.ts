...
const baseUrl = process.env.COMPONENTS_REGISTRY_URL ?? "https://eldoraui.site";
const proBaseUrl = process.env.PRO_REGISTRY_URL ?? "https://.eldoraui.site";
...
export async function getRegistryIndexEldoraUI(env?: boolean) {
  try {
    ...
    console.error(error);
    throw new Error("Failed to fetch components from Eldora UI registry.");
  }
}
...
export async function resolveTreeWithShadcn(
...
): Promise<{ shadcnTree: theTree; eldorauiTree: theTree }> {
  const shadcnTree: theTree = [];
  const eldorauiTree: theTree = [];
  ...
  for (const name of names) {
    if (!calledByShadcn) {
      ...
      entry && eldorauiTree.push(entry);

      if (entry && entry.registryDependencies) {
        const {
          eldorauiTree: eldorauiTreeDependencies,
        ...
        shadcnTree.push(...shadcnTreeDependencies);
        eldorauiTree.push(...eldorauiTreeDependencies);
      }
    } else {
  ...
  return {
    ...
    eldorauiTree: eldorauiTree.filter(
      (component, index, self) =>
        self.findIndex((c) => c.name === component.name) === index,
    ),
  };
