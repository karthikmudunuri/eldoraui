        ...
        item.href && !item.disabled ? (
          ...
          >
            {pathname === item.href && (
              <motion.div
                layoutId={groupId}
                className="absolute inset-0 rounded-r-md border-l-2 border-[#e879f9] bg-[#f0abfc] dark:bg-[#a21caf] "
                initial={false}
                transition={{
                  type: "spring",
                  stiffness: 350,
            ...
            <span className="relative z-10 shrink-0">{item.title}</span>
             {item.no && (
              <span className="relative z-10 ml-2 rounded-md bg-muted px-1.5 py-0.5 text-xs leading-none text-[#000000] no-underline group-hover:no-underline dark:text-[#ffffff] ">
                {item.no}
              </span>
            )}
            {item.label && (
