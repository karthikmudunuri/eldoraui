      ...
      {items.map((item, index) => (
        <div key={index} className="flex flex-col gap-1">
          <h4 className="rounded-md px-2 py-1 text-sm font-semibold">
            {item.title}{" "}
            {item.label && (
              <span className="ml-2 rounded-md bg-[#d946ef] px-1.5 py-0.5 text-xs font-normal leading-none text-[#000000] no-underline group-hover:no-underline dark:bg-[#f0abfc]">
                {item.label}
              </span>
            )}
          </h4>
      ...
      {items.map((item, index) =>
        item.href && !item.disabled ? (
          <Link
            ...
            className={cn(
              "group relative flex h-8 w-full items-center justify-between rounded-lg px-2 font-normal text-foreground underline-offset-2 hover:bg-[#f0abfc] dark:hover:bg-[#d946ef]",
          ...
          >
            {pathname === item.href && (
              <motion.div
                ...
                className="absolute inset-0 rounded-lg border-[#e879f9] bg-[#f0abfc] dark:bg-[#a21caf]"
                initial={false}
                transition={{
                  type: "spring",
                  stiffness: 350,
            ...
            <div>
              {item.label && (
                <span className="relative z-10 ml-2 rounded-md bg-[#d946ef] px-1.5 py-0.5 text-xs leading-none text-[#000000] no-underline group-hover:no-underline dark:bg-[#f0abfc]">
                  {item.label}
                </span>
              )}
              {item.paid && (
