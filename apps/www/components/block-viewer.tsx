    ...
    <div className="hidden w-full items-center gap-2 pl-2 md:pr-6 lg:flex">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            copyToClipboard(`pnpm dlx shadcn@latest add ${item.name}`)
          }}
        >
          {isCopied ? <Check /> : <Terminal />}
          <span>pnpm dlx shadcn@latest add {item.name}</span>
        </Button>
        <Separator orientation="vertical" className="mx-1 !h-4" />
        <OpenInV0Button url={`https://eldoraui.site/r/${item.name}.json`} />
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-col gap-2 lg:hidden">
      ...
      {item.meta?.mobile === "component" ? (
        ...
        <div className="overflow-hidden rounded-xl border">
          <Image
            src={`/blocks/${item.name}/light.png`}
            alt={item.name}
